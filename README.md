...
The nodes use the `imageNet` and `detectNet` objects from the [`jetson-inference`](https://github.com/dusty-nv/jetson-inference) library and [NVIDIA Two Days to a Demo](https://developer.nvidia.com/embedded/twodaystoademo) tutorial.
...
* [Running the `imagenet-console` Program on Jetson](https://github.com/dusty-nv/jetson-inference#using-the-console-program-on-jetson)

## ROS

Install `ros-base` package following these directions:
...
Then, create a Catkin workspace (`~/catkin_ws`) with these steps:
...
Navigate into your Catkin workspace and clone and build `ros_deep_learning`:
...
First, to stream some image data to process, open another terminal and start an [`image_publisher`](http://wiki.ros.org/image_publisher) node, which loads a specified image from disk.  We tell it to load one of the test images that come with `jetson-inference`, but you can substitute your own images here as well:
...
```
Here, we remap imagenet's `image_in` input topic to the output of the `image_publisher`, and tell it to load the GoogleNet model using the node's `model_name` parameter.
...
Kill the other nodes you launched above, and start publishing a new image with people in it:
...
Or if you can prefer, you can run the model that detects dogs instead:
...
To use the [`vision_msgs/Detection2DArray`](http://docs.ros.org/melodic/api/vision_msgs/html/msg/Detection2DArray.html) message output of the node, subscribe to the `detectnet/detections` topic:
